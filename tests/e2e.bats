...
setup() {
	...
	export CWD=$TMP/work
	export SID=e2e-1
	mkdir -p "$CWD" "$CLAUDETOGGLE_HOME/toggles/coauth" "$CLAUDETOGGLE_HOME/toggles/devlog"
	cat >"$CLAUDETOGGLE_HOME/toggles/coauth/toggle.sh" <<'EOF'
TOGGLE_API=1
TOGGLE_NAME=coauth
...
EOF
	cat >"$CLAUDETOGGLE_HOME/toggles/coauth/coauth.md" <<'EOF'
---
description: Toggle coauth
---
<!-- coauth-marker -->
EOF
	cat >"$CLAUDETOGGLE_HOME/toggles/devlog/toggle.sh" <<'EOF'
TOGGLE_API=1
TOGGLE_NAME=devlog
...
EOF
	cat >"$CLAUDETOGGLE_HOME/toggles/devlog/devlog.md" <<'EOF'
---
...
@test "install creates all expected symlinks and settings entries" {
...
@test "/devlog flips silently (no announce); statusline reflects custom indicator" {
	...
	[ -z "$out" ]
	[ -f "$CLAUDETOGGLE_HOME/devlog/sessions/$SID" ]
